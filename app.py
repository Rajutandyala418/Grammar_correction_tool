...
OPENAI_KEY = "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"

# ✅ Initialize OpenAI client (same style as your working test script)
client = OpenAI(api_key=OPENAI_KEY) if OPENAI_AVAILABLE else None
...
def correct_pronouns(text):
    ...
    if not gender:
        return text
    replacements = {
        "he": PRONOUNS[gender]["subject"],
        "she": PRONOUNS[gender]["subject"],
        "him": PRONOUNS[gender]["object"],
        "her": PRONOUNS[gender]["object"],
...
def correct_sentence_gpt(text):
    ...
    try:
        response = client.responses.create(
            model="gpt-4o-mini",
            input=f"Correct the following sentence grammatically. Return only the corrected sentence:\n{text}",
        )
...
def detect_tense(text):
    ...
    # Simple Past (no auxiliaries but past tense verb)
    if any(t.tag_ == "VBD" for t in doc):
        return "Past Simple"

    # Present Simple (base/3rd person singular verbs, no auxiliaries)
...
def process_text():
    try:
        ...
        print("Processed /process_text request")
